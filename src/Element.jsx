...
class Element extends React.Component {
  ...
  render() {
    ...
    return <div className={"canvasElement " + id}>
      <div className="canvasContent">
        ...
        {Object.entries(bullets).map ((item) => {
          return <li className="bullet" key={item[0]}>
            {item[1]}
            <button className='verwijderButton' type="button" onClick={this.removeBullet.bind(this, item[0], id, canvas)}>
      ...
      <button className='toevoegButton' type="button" onClick={this.setBullet.bind(this, id, naam, canvas)}>Voeg bullet toe <i className="fa-solid fa-plus"></i></button>
      <button className='vergrootButton' type="button" onClick={this.vergrootElement.bind(this, id, canvas)}><h2><i className="fa-solid fa-maximize"></i></h2></button>
    </div>;
  }
  ...
  setBullet(id, naam, canvas){
    ...
    canvas.setState({bulletAdder : newBulletAdder}, bulletDialog.showModal());
  }
  ...
  vergrootElement(id, canvas){
    ...
    canvas.setState({
      ...
      document.getElementById("presentationDialog").showModal()
    );
  }

  removeBullet(bulletId, id, canvas){
    bulletId = parseInt(bulletId);
    let oldElementData = canvas.state.data[id];
    let deel1 = oldElementData.slice(0, bulletId);
    let deel2 = oldElementData.slice(bulletId + 1, oldElementData.length)
